...
from flask_cors import CORS, cross_origin #it is required when deploying on a cloud platform
import pandas as pd
from mongoDBOperations import MongoDBManagement
from FlipkratScrapping import FlipkratScrapper
from selenium import webdriver #selenium to control the clicks
from webdriver_manager.chrome import ChromeDriverManager

rows = {} #creating blank dictionary
collection_name = None

logger = getLog('flipkrat.py') #calling function getlog with object ,

free_status = True #creating a flag
...
@app.route('/', methods=['POST', 'GET']) #creating a route
@cross_origin() #it will be used if deploying on a cloud of another region
def index():
    if request.method == 'POST': #via post means in a body(to hide actual data) not url
        global free_status #CREATING A GLOBAL VARIABLE
